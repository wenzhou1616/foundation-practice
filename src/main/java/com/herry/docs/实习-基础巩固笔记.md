...
HTTP（Hypertext Transfer Protocol）是一种用于在网络上进行数据通信的协议。它是一种无状态、面向请求响应的协议，常用于客户端和服务器之间的通信。

HTTP的主要目标是支持Web上的超文本传输和资源访问。它使用URL（统一资源定位符）作为定位资源的标识，并使用请求方法（如GET、POST）来定义对资源的操作。客户端通过发送HTTP请求到服务器，服务器则根据请求返回相应的HTTP响应。

HTTP协议基于TCP/IP协议栈，使用可靠的传输层连接（通常是基于TCP的）来传输数据。它使用明文文本格式进行通信，包括请求头、请求体、响应头和响应体等部分。
...
HTTP是一种不保存状态，即无状态(stateless)协议。HTTP协议自身不对请求和响应之间的通信状态进行保存。也就是说在HTTP这个级别，协议对于发送过的请求或响应都**不保存**（持久化）。
...
https://blog.csdn.net/alokka/article/details/115981135
