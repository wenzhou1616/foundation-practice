...
<img src="images/image-20231214012910258.png" alt="image-20231214012910258" style="zoom: 67%;" />
...
- 类 URL 和 URLConnection 提供了最高级网络应用。用URL 的网络资源的位置来同一表示 Internet 上各种网络资源。通过URL对象可以创建当前应用程序和 URL 表示的网络资源之间的连接，这样当前程序就可以读取网络资源数据，或者把自己的数据传送到网络上去。

# RESTful API

## 资源表述

在RESTful API中，一个很重要的概念就是“资源表述”。这是指一种可以识别和处理的数据格式，如JSON、XML、YAML等。对于不同的资源请求，服务器可以根据请求头中的`Accept`字段来提供相应的媒体类型。服务器也可以通过`Content-Type`字段来指定请求体的媒体类型。



如果你需要在你的API中使用多种媒体类型，你可以基于客户端的请求头来决定服务端响应的格式。



例如，当请求头为`Accept: application/json`，你的API可以返回JSON格式的数据；当请求头为`Accept: application/xml`，你的API就应返回XML格式的数据。



```java
app.get('/api/resource', function (req, res) {
  var requestedType = req.get('Accept');

  if(requestedType === 'application/json') {
    res.json({ message: '您请求的是JSON格式！' });
  } else if(requestedType === 'application/xml') {
    res.set('Content-Type', 'application/xml');
    res.send('<message>您请求的是XML格式！</message>');
  } else {
...
public class TableManagerTest {
    ...
    public static void main(String[] args) {
        TableManager excelManager = new TableManager();
//        List<TableManager.RowData> rowData = excelManager.readExcel("table01.xlsx");
        MyMethodInterceptor myMethodInterceptor = new MyMethodInterceptor(excelManager);
        TableManager proxyExcelManager = (TableManager)myMethodInterceptor.getProxy();
        List<TableManager.RowData> rowData = proxyExcelManager.readExcel("table01.xlsx");
