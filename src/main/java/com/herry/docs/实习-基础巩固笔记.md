...
最开始我们只能静态代理，编写代理类，实现同样的接口，调用目标对象的方法，在方法开始前或结束后额外做一些操作，但是假如有10个接口就要定义10个代理类，这样会导致出现大量冗余代理类，接口修改了，代理类也要改，不便于维护。
...
线程是操作系统能够进行运算调度的最小单位，一个线程包含了在执行过程中需要的各种信息，如程序计数器、寄存器集合、堆栈和一组资源等，因此每个线程都可以独立地执行不同的任务。

单线程是指在程序执行过程中只有一个线程在运行。在单线程模型中，程序按照顺序依次执行，每个任务必须等待前一个任务完成后才能开始执行。单线程的程序的优点是程序简单，便于维护、占用资源更少、线程安全。单线程的程序的缺点无法利用处理器的多个核心，性能差，效率低、无法并发执行、可能会阻塞，延迟。

多线程是指在程序执行过程中有多个线程同时运行。在多线程模型中，有多个线程同时执行，不同的线程可以执行不同的任务。多线程克服了单线程的缺点，但是多线程引入了新的问题：数据不一致、死锁、上下文切换开销、程序复杂等。

总而言之，单线程就是一个线程执行一多个任务，多线程就是多个线程执行多个任务，多线程完美解决了单线程效率低的问题，但是引入了新的问题，如：数据不一致、死锁、程序复杂等。
