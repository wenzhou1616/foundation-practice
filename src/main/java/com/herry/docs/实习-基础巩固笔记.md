...
假设现在要做一件事，就是在所有的实现类调用`wakeup()`前增加一行输出`早安~`，调用`sleep()`前增加一行输出`晚安~`。那我们只需要编写2个代理类`PersonProxy`和`AnimalProxy`：
...
最开始我们只能静态代理，编写代理类，实现同样的接口，调用目标对象的方法，在方法开始前或结束后额外做一些操作，但是假如有100个接口就要定义100个代理类，这样会导致出现大量冗余代理类，接口修改了，代理类也要改，不便于维护。
...
**线程**是操作系统能够进行运算调度的最小单位，一个线程包含了在执行过程中需要的各种信息，如程序计数器、寄存器集合、堆栈和一组资源等，因此每个线程都可以独立地执行不同的任务。

**单线程**是指在程序执行过程中只有一个线程运行。在单线程模型中，程序按照顺序依次执行，每个任务必须等待前一个任务完成后才能开始执行。单线程的程序的优点是程序简单，便于维护、占用资源更少、线程安全。单线程的程序的缺点必须顺序执行任务，一次只能执行一个，性能差、效率低、不支持并发执行、可能会阻塞、延迟。

**多线程**是指在程序执行过程中有多个线程同时运行。在多线程模型中，有多个线程同时执行，每个线程都可以执行不同的任务，因此多线程克服了单线程的缺点，但是多线程引入了新的问题：数据不一致、死锁、程序复杂等。

总而言之，单线程就是一个线程执行多个任务，多线程就是多个线程执行多个任务，由于单线程有效率低的缺点，从而提出了多线程，   多线程可以同时调度多个线程来执行多个任务，解决了效率低的问题，但是引入了新的问题：数据不一致、死锁、程序复杂等。

**进程**是操作系统中正在运行的程序，比如微信、QQ、浏览器，一个进程里有多个线程，就对应着在微信中可以同时下载文件、聊天、接收消息等。

**并发**是指多个任务在**同一时间段内交替执行**，由于CPU切换的速度极快，人感知不到，因此在人看来是同时执行多个任务。

**并行**是指多个任务在**同一时刻执行**，一般发生在多核处理中，并行才是真正的同时执行多个任务。

在现实的操作系统中，由于任务数远超于处理器的核心数，因此并发和并行同时存在于操作系统中。

在后续的网络编程、数据库、Web开发等都会用上多线程，因此多线程这部分很重要。
...
public class MyThread extends Thread {
    @Override
    public void run() {
        System.out.println("Thread - run new thread!");
    }
}
```
...
继承 Thread 类，重写 run 方法，然后创建对象，调用 start 方法就能创建线程。

![image-20231206152519637](images/image-20231206152519637.png)

这里有一个经典的问题，调用 Run 方法不行吗？

调用 Run 方法不会创建线程，只能调用了一个重写过的方法而已，调用 start 方法才会创建新线程。
...
为了克服以上的缺点，更常用的创建线程的方式是实现Runnable接口。
...
实现 Runnable 接口，创建对象，作为参数传入Thread的构造函数，然后调用start创建线程。
...
1. 创建（Creation）：线程被创建后，会分配所需的内存空间并初始化其内部数据结构。在这个阶段，操作系统将会为新线程分配一个唯一的标识符，称为线程ID。

2. 就绪（Ready）：在就绪状态下，线程已经准备好运行，但还未被分配到处理器时间片。此时，线程将被添加到操作系统的就绪队列中，等待处理器的调度。

3. 运行（Running）：当线程被分配到处理器时间片后，它将进入运行状态，并开始执行其指定的任务。在这个阶段，线程可以访问所有所需的资源，包括内存、I/O设备等。

4. 阻塞（Blocked）：在某些情况下，线程可能会失去对某些资源的访问权限。例如，线程可能需要等待I/O操作完成或等待其他线程释放共享资源。在这种情况下，线程将进入阻塞状态，并从操作系统的就绪队列中移除。

5. 结束（Termination）：当线程完成其指定的任务或因某种原因无法继续执行时，它将进入结束状态。在这个阶段，线程将释放其占用的所有资源，并从操作系统的线程表中移除。

   

在 Java 中的线程的生命周期有 6 种状态：
...
和操作系统相比少了就绪，多了等待和计时等待。

![image-20231206235956507](images/image-20231206235956507.png)

这六种状态在 java.lang.Thread.State 的枚举类中有定义：
...
- **NEW（新建）**：线程对象刚被创建，但是并未启动，就是还没调用start方法。
- **RUNNABLE（可运行）**：一旦调用 start 方法就进入可运行状态。但是这并不意味着线程能直接运行，因为还要等操作系统分配CPU资源，因此这个状态叫做“可运行”而不是“运行”。在Java中没有区分就绪和可运行状态，因为这之间的时间非常短暂，无法区分。
- **BLOCKED（阻塞）**：线程获取对象锁失败就会进入阻塞状态，即获取锁失败就会阻塞。
  - 比如，线程A与线程B代码中使用同一锁，如果线程A获取到锁，线程A进入到Runnable状态，那么线程B就进入到Blocked锁阻塞状态。
- **WAITING（等待）**：线程停止执行程序，等待其他线程唤醒，才会继续执行。
...
- **TIMED_WAITING（计时等待）**：有时间的等待，时间到或被其他线程唤醒就继续执行。
  - 当前线程执行过程中遇到Thread类的`sleep`或`join`，Object类的`wait`，LockSupport类的`park`方法，并且在调用这些方法时，**设置了时间**，那么当前线程会进入TIMED_WAITING，直到时间到。

- **Teminated（终止）**：表示线程的生命周期结束了，线程运行终止。
