...
ArrayList 的指定位置的添加和删除，本质上其实是数组的指定位置添加和删除，而数组的指定位置的添加和删除是要移动元素的，添加往后移，删除往前移，ArrayList 的对元素移动是通过`arraycopy` 方法在同一个数组上进行复制实现的。

**集合的复制：**

浅拷贝：

浅拷贝是指在复制对象时，对于基本数据类型的属性会全部复制，而引用数据类型的属性只复制引用，即一个对象修改了，另一个对象会受到影响。

深拷贝：

深拷贝是指在复制对象时，无论是基本数据类型还是引用数据类型都会全部复制，和原始对象相互独立，一个修改了不会影响另外一个。
...
List<People> newList = new ArrayList<>(sourceList);
System.out.println(sourceList.toString()); // [People{name='张三'}]
System.out.println(newList.toString()); // [People{name='张三'}]

people1.setName("李四");
System.out.println(sourceList.toString()); // [People{name='李四'}]
System.out.println(newList.toString()); // [People{name='李四'}]
```

2.使用addAll() 方法，浅拷贝：

```java
Collections.copy()People people1 = new People("张三");
...
4.克隆对象。如果集合中的元素类实现了 `Cloneable` 接口并重写了 `clone()` 方法，那么可以通过调用元素对象的 `clone()` 方法来实现深拷贝。
