...
6. 解决哈希冲突（两个不同的键映射到了相同的桶位置）：HashMap 使用链地址法解决哈希冲突，流程如下：
...
首先用key的哈希值经过扰动函数得到新的哈希值，目的是让哈希值分布更均匀一些，减少哈希冲突，然后用数组长度减一与哈希值进行与运算（`(n - 1) & hash`） ，得到元素要插入的位置，如果当前位置没有元素则直接插入，如果有元素，就先判断哈希值是否相同，然后用 equals 方法判断对象的内容是否相同，如果都相同，则直接覆盖，如果不相同则用拉链法解决哈希冲突。
