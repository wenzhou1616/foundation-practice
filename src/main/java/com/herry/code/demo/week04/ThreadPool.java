...
//                        handler.rejectedExecution(task, this);
    ...
    public static void main(String[] args) {
        ...
        ThreadPool pool = new ThreadPool(2, 5, 1, TimeUnit.SECONDS, workQueue, threadFactory, handler);
        ...
        while (true) {
            try {
                Thread.sleep(1000);
                System.out.println("ThreadPool status: " + pool.getStatus());
                if ("Terminated".equals(pool.getStatus())) {
                    break;
                }
            } catch (InterruptedException e) {
                e.printStackTrace();
